...
_encrypt_text:
    ...
    ; открытие файла const_filename_plaintext
    mov eax, 5
    ...
    _loop_read_plaintext_file:
        ...
        mov eax, dword [val_ptr_file_plaintext]
        mov ebx, val_buffer
        mov ecx, 4096
        int 80h
        ...
        ; используем для хранения текущего символа ключа
        xor esi, esi
        ...
        ; 2) иначе пропускаем, причем в этом случае все равно переходим к следующему символу ключа (пусть будет так)
        ; В цикле нельзя использовать регистры ecx - отвечает за работу цикла (подробнее стр. 5) и
        ; eax - хранит длинну считанного текста (можно сохранять перед использованием, а потом восстановливать)
        _loop_encrypt_buffer:
            ...
            pop ebx
            pop eax
            ; закончили брать по модулю
            ; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


            ; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ; начало шифрования
            ; проверка что символ - буква или цифра
            ; а иначе пропуск этого шифрования
